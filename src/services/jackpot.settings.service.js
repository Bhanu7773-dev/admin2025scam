...
const GAME_SETTINGS_COLLECTION = "jackpot_game_settings"
/**
 ...
 */
export const getJackpotGameRates = async () => {
    const ratesSnapshot = await db.collection(GAME_SETTINGS_COLLECTION).get();
    if (ratesSnapshot.empty) {
        console.warn("No documents found in the jackpot_game_settings collection.");
        return {};
    }
 ...
 */
export const updateJackpotGameRates = async (newRates) => {
    console.log("Received rates object from client:", newRates);
    if (!newRates || typeof newRates !== 'object') {
        throw new Error("updateJackpotGameRates(): newRates object is required.");
    }
    ...
    try {
        await batch.commit();
        return { success: true, message: "Jackpot Game rates updated successfully." };
    } catch (error) {
        console.error("Error committing jackpot game rates batch:", error);
        throw new Error("Failed to update jackpot game rates.");
    }
};
